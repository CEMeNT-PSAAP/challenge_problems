...
n_segs = 8
v_grid = np.zeros((n_segs+1, 3))
v_duration = (tf-to)/n_segs * np.ones(n_segs+1)
for i in range(n_segs):
    v_grid[i+1, 2] = v_grid[i, 2] + g * ((tf-to)/n_segs)

sz6.move(v_grid, v_duration)
sz7.move(v_grid, v_duration)
...
mcdc.cell(+sx5 & -sx6 & +sy5 & -sy6 & +sz6 & -sz7, core) # slug
...
Nz = 200
Nt = 100
x_grid = np.linspace(-core_width, core_width, Nx+1)
...
# Setting
mcdc.setting(N_particle=1e6, N_batch=10, active_bank_buff=10000, progress_bar=False)
#mcdc.branchless_collision()
#mcdc.time_census(np.linspace(0.7,0.8,100))
#mcdc.population_control('combing-weight')
